...
function Header() {
  ...
  return (
    <div
      className={`fixed top-10 flex flex-row items-center shadow-lg z-50
                 shadow-blue40 rounded-3xl gap-4 bg-gray10-transparent dark:bg-transparent
                 h-[40px] transition-all duration-700 backdrop-blur-sm  ${
    ...
    >
      ...
      <div
        className={`flex items-center justify-center gap-6 md:gap-12 transition-all duration-1000 ${
          isExpanded ? "ml-20 md:ml-32" : "ml-0"
        }`}
      >
        {isExpanded && (
          <a href="#about" className="mt-2">
            <CustomTooltip  element={<Info width={28} height={28} className="text-blue70 dark:text-gray30 ml-10" />} info="About" />
          </a>
        )}
        {isExpanded && (
          <a href="#projects" className="mt-2">
            <CustomTooltip element={<MonitorCog width={28} height={28} className="text-blue70 dark:text-gray30" />} info="Projects" />
          </a>
        )}
        {isExpanded && (
          <a href="#blogs" className="mt-2">
            <CustomTooltip element={<SquareLibrary width={28} height={28} className="text-blue70 dark:text-gray30" />} info="Blogs" />
          </a>
        )}
      </div>
    </div>
