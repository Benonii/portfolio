...
function Header() {
    ...
    return (
        <div 
            className={
                `fixed top-10 flex flex-row items-center backdrop-blur-sm
                 shadow-lg shadow-blue40 rounded-3xl gap-4 bg-gray10-transparent dark:bg-transparent
                 h-[40px] transition-all duration-1000 ${isExpanded ? 'w-72 md:w-96 lg:w-[400px]' : 'w-28'
                 }`
            }
        >
            <div 
            ...
            <div className={`flex items-center gap-6 md:gap-12 transition-all duration-1000 ${
                ...
                {isExpanded && <a href="#blogs"><SquareLibrary width={28} height={28} className='text-blue70 dark:text-gray30' /></a>}     
            </div>
        </div>
    )
}
